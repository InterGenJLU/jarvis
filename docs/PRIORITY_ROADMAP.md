...
| 19 | **Web query memory** — ~~SQLite DB of last 100 web queries + results, "what did we look up?"~~ **Largely superseded by Artifact Cache Phase 5 (cross-session retrieval).** Session summaries + cold-tier artifacts already persisted with embeddings. Remaining: recall handler in conversation_router | 2-3 hours | Artifact cache stores all interaction data with semantic embeddings. Phase 5 = query path | `docs/INTERACTION_ARTIFACT_CACHE_DESIGN.md`. Phases 1-4 complete |
