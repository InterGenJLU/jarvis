...
| H8 | **Presence greeting latency + prosody review** — "Welcome back sir. Ready when you are." exchange analysis. Timings: face detect (08:02:03) → TTS speak greeting (08:02:03, ~0ms routing) → TTS playback completes (08:02:07, ~4s Kokoro) → user responds "good morning" (08:02:12, ~5s user gap) → CAL briefing composed via 4B (08:02:14, ~2s routing+compose) → briefing TTS (08:02:14). Total greeting-to-briefing: ~11s. Also: "Ready when you are" has wrong prosodic pitch from Kokoro — sounds unnatural. Explore G2P override or alternate phrasing. | 1-2 hours | Review latency budget for presence→briefing pipeline. Kokoro prosody issue may need pronunciation override or phrase replacement. |
