...
_mock_speechbrain = MagicMock()
sys.modules["speechbrain"] = _mock_speechbrain
sys.modules["speechbrain.inference"] = MagicMock()
sys.modules["speechbrain.inference.speaker"] = MagicMock()
sys.modules["torchaudio"] = MagicMock()
sys.modules["torch"] = MagicMock()

# EMBEDDING_DIM must be importable
from core.speaker_id import EMBEDDING_DIM
...
def make_deterministic_encoder(embed_fn=None):
    ...
    Default: normalised hash of the first 100 samples → 192-dim vector.
    This ensures different audio → different embeddings, same audio → same embedding.
    """
    mock_encoder = MagicMock()

    def default_embed(waveform):
        # waveform is a torch tensor (batch, time) — use numpy for mock
        if hasattr(waveform, 'numpy'):
            audio = waveform.squeeze().numpy()
        else:
            audio = np.asarray(waveform).flatten()
        rng = np.random.RandomState(int(abs(audio[:10].sum()) * 1000) % (2**31))
        emb = rng.randn(EMBEDDING_DIM).astype(np.float32)
        emb /= np.linalg.norm(emb)
        # Return as mock tensor with shape (1, 1, 192)
        result = MagicMock()
        result.squeeze.return_value = result
        result.cpu.return_value = result
        result.numpy.return_value = emb
        return result

    mock_encoder.encode_batch = embed_fn or default_embed
    return mock_encoder
...
def make_speaker_id(tmpdir, threshold=0.85, embed_fn=None):
    ...
    # Inject mock encoder to skip lazy-loading SpeechBrain
...
def test_part1():
    ...
    # --- Test: valid audio → 192-dim non-zero array ---
    sid, _ = make_speaker_id(tmpdir)
    audio = make_audio(3.0)
    emb = sid.extract_embedding(audio)
    assert_eq("embed: output is 192-dim", len(emb), EMBEDDING_DIM)
    ...
    assert_true("embed short: returns 192-dim zeros",
                np.all(emb_short == 0) and len(emb_short) == EMBEDDING_DIM,
    ...
    assert_eq("embed dtype: output is 192-dim", len(emb_f64), EMBEDDING_DIM)
    ...
    assert_eq("embed resample: output is 192-dim", len(emb_44k), EMBEDDING_DIM)
...
def test_part2():
    ...
    if "alice" in sid._cache:
        ...
        assert_eq("enroll: cache embedding is 192-dim", len(cached_emb), EMBEDDING_DIM)
    ...
    def fixed_embed_factory():
        call_count = [0]
        fixed_embeddings = [
            np.array([1.0, 0.0] + [0.0] * (EMBEDDING_DIM - 2), dtype=np.float32),
            np.array([0.0, 1.0] + [0.0] * (EMBEDDING_DIM - 2), dtype=np.float32),
        ]
        def fixed_embed(waveform):
    ...
    sid4, pm4 = make_speaker_id(tmpdir, embed_fn=fixed_embed_factory())
    ...
    if "carol" in sid4._cache:
        ...
        expected_avg = np.array([0.5, 0.5] + [0.0] * (EMBEDDING_DIM - 2), dtype=np.float32)
...
def test_part3():
    ...
    def make_embed_fn(target_emb):
        def embed_fn(waveform):
            result = MagicMock()
            result.squeeze.return_value = result
            result.cpu.return_value = result
            result.numpy.return_value = target_emb.copy()
            return result
        return embed_fn
    ...
    sid2, _ = make_speaker_id(tmpdir, threshold=0.85, embed_fn=make_embed_fn(emb_a))
    ...
    sid4, _ = make_speaker_id(tmpdir, threshold=0.5, embed_fn=make_embed_fn(emb_a))
    ...
    sid4._encoder = make_deterministic_encoder(make_embed_fn(emb_b))
    ...
    mixed = np.zeros(EMBEDDING_DIM, dtype=np.float32)
    mixed[0] = 0.7
    mixed[1] = 0.714
    mixed /= np.linalg.norm(mixed)

    sid5, _ = make_speaker_id(tmpdir, threshold=0.95, embed_fn=make_embed_fn(mixed))
...
def test_part4():
    ...
    sid, _ = make_speaker_id(tmpdir, threshold=0.85, embed_fn=make_embed_fn(emb_a))
    ...
    sid._encoder = make_deterministic_encoder(make_embed_fn(emb_b))
...
def test_part5():
    ...
    emb_b = np.random.randn(EMBEDDING_DIM).astype(np.float32)
    ...
    sid._cache["stale_user"] = (np.zeros(EMBEDDING_DIM), "test")
    sid.load_embeddings()
    ...
    sid2, pm2 = make_speaker_id(tmpdir)
    emb_c = np.random.randn(EMBEDDING_DIM).astype(np.float32)
    ...
    sid2._cache["dave"] = (np.zeros(EMBEDDING_DIM), "test")
